...
                    # --- PERBAIKAN DIMULAI DI SINI ---
                    # 1. Panggil .plot() dengan line_width yang lebih tebal agar Bounding Box lebih terlihat
                    results = model(image, conf=confidence_threshold)
                    plot_result = results[0].plot(line_width=4) # Diubah dari .plot() menjadi .plot(line_width=4)
                    
                    if plot_result is not None:
                        result_img_rgb = cv2.cvtColor(plot_result, cv2.COLOR_BGR2RGB)
                        
                        # 2. Hapus placeholder.info() sebelum menampilkan konten baru
                        placeholder.empty()
                        
                        with placeholder.container():
                            st.subheader("🎯 Hasil Deteksi")
                            # 3. st.image akan otomatis tampil dalam ukuran besar (use_container_width=True)
...
                    else:
                        # Jika plot_result None, kita masih harus menghapus info
                        placeholder.empty()
